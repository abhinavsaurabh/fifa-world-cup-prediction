...
for i, group in enumerate(groups):
    for o in rr_outcomes(int(len(group))):
...
def next_level(bracket):
    m, _, n = bracket.shape
    new_bracket = np.zeros((int(m/2), 2, n))
    for g, p1, p2 in np.ndindex((m,n,n)):
        #odds vs evens
        ngi = g % 2
        game_prob = bracket[g, 0, p1] * bracket[g, 1, p2]
        new_bracket[int(g/2), ngi, p1] += wl[p1,p2] * game_prob
        new_bracket[int(g/2), ngi, p2] += wl[p2,p1] * game_prob
